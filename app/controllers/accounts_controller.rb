class AccountsController < ApplicationController
  ...
  def withdraw
    ...
    # return render json: { message: "Insufficient Funds"}, status: :unprocessable_entity if account.balance < amount
    # return render json: { message: "Invalid Ammount"}, status: :unprocessable_entity if amount.to_i <= 0
  ...
  def create
    ...
    if account.save
      ...
      render json: { message: "Failed", errors: accounts.errors}, status: :unprocessable_entity
    end
  end
  private
  def accounts_params
    params.require(:account).permit(:first_name, :last_name, :middle_name, :email, :contact_number, :birthdate, :home_address, :zip_code, :city, :gender, :account_number, :account_type, :balance)
  end
  def accounts
    Account.all
  end
  ...
  def set_errors(amount,totransfer=nil, type="Deposit")
    return render json: { errors: {amount: ["Insufficient Funds"]}}, status: :unprocessable_entity if account.balance < amount 
    return render json: { errors: {amount: ["Please enter a valid number"]}}, status: :unprocessable_entity if amount.to_i <= 0 
  rescue ActiveRecord::RecordNotFound
    return render json: { errors: {account_number: ["Please enter a valid account number"]}}, status: :unprocessable_entity 
